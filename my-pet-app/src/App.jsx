 ...
 * - TopBar includes Pet Switcher, Manage, ＋ Add Pet, ℹ️ Info
 * - Hotspots on background open Memories / Notes / Goals / Flashcards
 */
...
// Hearts derived from goals (10 hearts = 100%)
...
const speciesAvatar = sp => (
  { cat: "🐱", dog: "🐶", panda: "🐼", fish: "🐟", fox: "🦊", bunny: "🐰", duck: "🦆", turtle: "🐢" }[sp] || "🐾"
);
...
function CatSVG({ facingLeft, bob }) {
  ...
  return (
    <svg viewBox="0 0 64 48" className="w-14 h-10 select-none pointer-events-none" style={{ transform: `scaleX(${sx}) translateZ(0)` }} aria-label="cat">
...
function DogSVG({ facingLeft, bob }) {
  const sx = facingLeft ? -1 : 1;
  return (
    <svg viewBox="0 0 64 48" className="w-14 h-10 select-none pointer-events-none" style={{ transform: `scaleX(${sx}) translateZ(0)` }} aria-label="dog">
      <rect x="10" y="20" width="38" height="16" rx="6" fill="#c4a484" />
      <rect x="40" y="14" width="14" height="12" rx="4" fill="#c4a484" />
      <rect x="42" y="10" width="6" height="6" rx="2" fill="#8b6b4c" style={{ transform: `translateY(${bob ? 0 : 1}px)` }} />
      <rect x="51" y="18" width="4" height="3" rx="1.5" fill="#333" />
      <circle cx="47" cy="19" r="1.4" fill="#1d1d1f" />
      <rect x="16" y="36" width="6" height="6" rx="2" fill="#ad8c6e" style={{ transform: `translateY(${bob ? 0 : 1}px)` }} />
      <rect x="28" y="36" width="6" height="6" rx="2" fill="#ad8c6e" style={{ transform: `translateY(${bob ? 1 : 0}px)` }} />
      <path d={`M 10 26 C 2 ${bob ? 18 : 20}, 2 ${bob ? 10 : 12}, 12 12`} stroke="#c4a484" strokeWidth="6" fill="none" strokeLinecap="round" />
    </svg>
  );
}
function PandaSVG({ facingLeft, bob }) {
  const sx = facingLeft ? -1 : 1;
  return (
    <svg viewBox="0 0 64 48" className="w-14 h-10 select-none pointer-events-none" style={{ transform: `scaleX(${sx}) translateZ(0)` }} aria-label="panda">
...
function FishSVG({ facingLeft, bob }) {
  const sx = facingLeft ? -1 : 1;
  return (
    <svg viewBox="0 0 64 48" className="w-14 h-10 select-none pointer-events-none" style={{ transform: `scaleX(${sx}) translateZ(0)` }} aria-label="fish">
      <ellipse cx="28" cy="26" rx="16" ry="10" fill="#5cc0ff" />
      <polygon points="12,26 4,20 4,32" fill="#49a7e6" style={{ transform: `translateX(${bob ? 0 : 1}px)` }} />
      <circle cx="36" cy="24" r="2" fill="#0b2533" />
      <polygon points="24,18 18,12 28,16" fill="#79d2ff" />
      <polygon points="30,34 24,38 34,34" fill="#79d2ff" />
    </svg>
  );
}
function FoxSVG({ facingLeft, bob }) {
  const sx = facingLeft ? -1 : 1;
  return (
    <svg viewBox="0 0 64 48" className="w-14 h-10 select-none" style={{ transform: `scaleX(${sx})` }} aria-label="fox">
      <rect x="12" y="20" width="36" height="16" rx="5" fill="#f97316" />
      <rect x="38" y="12" width="16" height="14" rx="4" fill="#f97316" />
      <polygon points="40,12 44,6 48,12" fill="#f97316" style={{ transform: `translateY(${bob ? 0 : 1}px)` }} />
      <polygon points="50,12 54,6 58,12" fill="#f97316" style={{ transform: `translateY(${bob ? 1 : 0}px)` }} />
      <rect x="42" y="16" width="12" height="6" rx="2" fill="#fff" />
      <circle cx="44" cy="18" r="1.2" fill="#111" /><circle cx="52" cy="18" r="1.2" fill="#111" />
      <path d={`M 10 28 c 0 0 -6 ${bob ? 0 : 2} -6 6 s 10 6 12 6`} stroke="#f97316" strokeWidth="6" fill="none" strokeLinecap="round" />
    </svg>
  );
}
function BunnySVG({ facingLeft, bob }) {
  const sx = facingLeft ? -1 : 1;
  return (
    <svg viewBox="0 0 64 48" className="w-14 h-10 select-none" style={{ transform: `scaleX(${sx})` }} aria-label="bunny">
      <rect x="12" y="22" width="34" height="14" rx="7" fill="#f4f4f5" />
      <rect x="38" y="12" width="14" height="12" rx="6" fill="#f4f4f5" />
      <rect x="40" y="4" width="4" height="12" rx="2" fill="#e5e7eb" style={{ transform: `translateY(${bob ? 0 : 1}px)` }} />
      <rect x="48" y="4" width="4" height="12" rx="2" fill="#e5e7eb" style={{ transform: `translateY(${bob ? 1 : 0}px)` }} />
      <circle cx="44" cy="18" r="1.1" fill="#111" /><circle cx="50" cy="18" r="1.1" fill="#111" />
      <rect x="46.5" y="20" width="2" height="1.5" rx="0.6" fill="#d4d4d8" />
      <circle cx="12" cy="30" r="3" fill="#e5e7eb" />
    </svg>
  );
}
function DuckSVG({ facingLeft, bob }) {
  const sx = facingLeft ? -1 : 1;
  return (
    <svg viewBox="0 0 64 48" className="w-14 h-10 select-none" style={{ transform: `scaleX(${sx})` }} aria-label="duck">
      <ellipse cx="28" cy="28" rx="16" ry="10" fill="#fde047" />
      <rect x="38" y="22" width="10" height="6" rx="3" fill="#f97316" />
      <circle cx="46" cy="22" r="2" fill="#f97316" />
      <circle cx="36" cy="24" r="1.6" fill="#111" />
      <polygon points="12,28 6,24 6,32" fill="#fde047" style={{ transform: `translateX(${bob ? 0 : 1}px)` }} />
    </svg>
  );
}
function TurtleSVG({ facingLeft, bob }) {
  const sx = facingLeft ? -1 : 1;
  return (
    <svg viewBox="0 0 64 48" className="w-14 h-10 select-none" style={{ transform: `scaleX(${sx})` }} aria-label="turtle">
      <ellipse cx="28" cy="28" rx="16" ry="11" fill="#10b981" />
      <rect x="12" y="28" width="6" height="6" rx="2" fill="#065f46" style={{ transform: `translateY(${bob ? 0 : 1}px)` }} />
      <rect x="22" y="36" width="6" height="6" rx="2" fill="#065f46" style={{ transform: `translateY(${bob ? 1 : 0}px)` }} />
      <rect x="34" y="36" width="6" height="6" rx="2" fill="#065f46" style={{ transform: `translateY(${bob ? 0 : 1}px)` }} />
      <rect x="40" y="24" width="10" height="8" rx="3" fill="#10b981" />
      <circle cx="50" cy="27" r="1.4" fill="#111" />
    </svg>
  );
}
...
function TopBar({ onToggleInfo, onNewPet, pets, activeId, onSwitch, onManage }) {
  return (
    <div
      className="sticky top-0 z-30 flex items-center justify-between px-3 h-10
                 bg-zinc-800 text-zinc-100 border-b border-zinc-700"
    >
      <div className="flex items-center gap-2">
        <span className="text-xs tracking-widest">VET PET</span>
        <span className="opacity-70 text-[11px]">· {pets.find(p => p.id === activeId)?.name ?? "Pet"}</span>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <button type="button" className="px-2 py-1 rounded hover:bg-zinc-700/60" title="Manage" onClick={onManage}>⚙️ Manage</button>
        <button type="button" className="px-2 py-1 rounded hover:bg-zinc-700/60" title="Add Pet" onClick={onNewPet}>＋ Add Pet</button>
        <button type="button" className="px-2 py-1 rounded hover:bg-zinc-700/60" title="Info" onClick={onToggleInfo}>ℹ️ Info</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex gap-1 text-[18px] leading-none">
      {Array.from({ length: max }).map((_, i) => (
        <span key={i} className={i < full ? "text-rose-500" : "text-zinc-400/60"}>❤</span>
      ))}
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative rounded-3xl bg-white/95 backdrop-blur shadow-2xl border border-zinc-200 w-[480px] max-w-[92vw] p-5 text-zinc-900">
        <div className="flex items-center justify-between mb-3">
          ...
          <button type="button" onClick={onClose} className="text-zinc-500 hover:text-zinc-700">✕</button>
        </div>

        <div className="mb-4"><HeartRow hearts={heartsFromGoals} /></div>

        <div className="flex gap-2 mb-4">
          <button type="button" onClick={onOpenGoals} className="px-3 py-1.5 rounded-full bg-rose-100 text-rose-600 text-sm">🎯 Goals</button>
          <button type="button" onClick={onOpenMemories} className="px-3 py-1.5 rounded-full bg-indigo-100 text-indigo-700 text-sm">🎞️ Memories</button>
          <button type="button" onClick={onPlay} className="px-3 py-1.5 rounded-full bg-amber-100 text-amber-700 text-sm">⭐ Play</button>
        </div>

        <div className="divide-y divide-zinc-200">
          <Row icon="🐾" label="Species" value={prettySpecies(pet.species)} />
          <Row icon="⏳" label="Age" value={`${Math.max(0, Math.floor((Date.now() - pet.createdAt) / 60000))} min`} />
          <Row icon="⚖️" label="Weight">
          ...
          <Row icon="⚡" label="Daily Memorize Progress" value={`${dailyPct}%`} />
          <Row icon="🌀" label="Scrolled Together" value={`${(pet.distanceKm ?? 0).toFixed(2)} km`} />
          {Object.entries(extra).map(([key, val]) => (
            <Row key={key} icon="📝" label={key} value={String(val)} />
          ))}
        </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-[560px] max-w-[96vw] rounded-3xl bg-white text-zinc-900 p-6 shadow-2xl border border-zinc-200">
        <div className="flex items-center justify-between mb-4">
          <div className="text-base font-semibold">🎯 Goals for {pet.name}</div>
          <button type="button" className="text-zinc-500 hover:text-zinc-700" onClick={onClose}>✕</button>
        </div>
...
function MemoriesPanel({ open, onClose, pet }) {
  ...
  const startSlideShow = (i = 0) => setViewer(i);
  const closeSlide = () => setViewer(null);
  const next = () => setViewer(v => (v + 1) % items.length);
  const prev = () => setViewer(v => (v - 1 + items.length) % items.length);

  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      <div className="absolute inset-0 bg-black/50" onClick={onClose} />
      <div className="relative w-[880px] max-w-[96vw] rounded-3xl bg-white text-zinc-900 p-6 shadow-2xl border border-zinc-200">
        <div className="flex items-center justify-between mb-4">
          <div className="text-base font-semibold">🎞️ Memories · {pet.name}</div>
          <button type="button" className="text-zinc-500 hover:text-zinc-700" onClick={onClose}>✕</button>
        </div>

        <div className="flex items-center gap-2 mb-4">
          <input type="file" multiple accept="image/*,video/*" onChange={onUpload} />
          <button type="button" onClick={() => items.length && startSlideShow(0)} className="px-3 py-2 rounded bg-zinc-900 text-white text-sm disabled:opacity-50" disabled={!items.length}>▶️ Start Slideshow</button>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 max-h-[50vh] overflow-auto pr-1">
          {items.map((it, i) => (
            <div key={it.id} className="group relative rounded-xl overflow-hidden border border-zinc-200 bg-zinc-50">
              {it.isVideo ? (
                <video src={it.url} muted playsInline className="w-full h-32 object-cover" />
              ) : (
      ...
      {viewer != null && items[viewer] && (
        <div className="absolute inset-0 z-[70] flex items-center justify-center bg-black/80">
          <button className="absolute top-4 right-4 text-white text-2xl" onClick={closeSlide}>✕</button>
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center">
      ...
      <div className="relative w-[900px] max-w-[96vw] rounded-3xl bg-white text-zinc-900 p-6 shadow-2xl border border-zinc-200">
        <div className="flex items-center justify-between mb-4">
          <div className="text-base font-semibold">📓 Notes · {pet.name}</div>
          <button type="button" className="text-zinc-500 hover:text-zinc-700" onClick={onClose}>✕</button>
        </div>
          ...
          {pets.map(p => (
            <div key={p.id} className="grid grid-cols-12 items-center gap-2 p-2 rounded-xl border border-zinc-200 bg-white/70">
              ...
              <input
                ...
                onChange={(e) => setLocalNames(s => ({ ...s, [p.id]: e.target.value }))}
                onBlur={() => renamePet(p.id, localNames[p.id] ?? p.name)}
                className="col-span-5 px-3 py-2 rounded border border-zinc-300 text-zinc-900 placeholder-zinc-500"
              />
              <select
                value={p.species}
                onChange={(e) => setSpecies(p.id, e.target.value)}
                className="col-span-3 px-2 py-2 rounded border border-zinc-300 text-zinc-900"
              >
                {SPECIES.map(s => <option key={s} value={s}>{speciesAvatar(s)} {prettySpecies(s)}</option>)}
              </select>
              ...
              <div className="col-span-1 flex items-center gap-1">
                <button type="button" onClick={() => movePet(p.id, 'up')} className="px-2 py-1 rounded border border-zinc-300 text-xs">▲</button>
                <button type="button" onClick={() => movePet(p.id, 'down')} className="px-2 py-1 rounded border border-zinc-300 text-xs">▼</button>
              </div>
...
const DEFAULT_VET_DECK = [
  { q: "Normal canine resting HR?", a: "60–120 bpm", tag: "Physio" },
  { q: "Cat core vaccine trio?", a: "FVRCP", tag: "Vaccines" },
  { q: "Pre-renal azotemia cause?", a: "↓ renal perfusion", tag: "Renal" },
...
function FlashcardGame({ open, onClose, deck, setDeck, onComplete, pet }) {
  ...
  useEffect(() => {
    if (open) {
      const order = [...deck].sort(() => Math.random() - 0.5);
      setShuffled(order); setIdx(0); setShowAns(false); setCorrect(0);
    }
  }, [open, deck]);
  ...
  const mark = isCorrect => {
    ...
    if (idx + 1 < total) {
      setIdx(idx + 1); setShowAns(false);
    } else {
      onComplete?.({ correct: isCorrect ? correct + 1 : correct, total });
      onClose();
    }
  };
  const addCard = () => {
    if (!q || !a) return;
    setDeck([...deck, { q, a, tag }]);
    setQ(""); setA(""); setTag("");
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-[680px] max-w-[96vw] rounded-3xl bg-white text-zinc-900 p-6 shadow-2xl border border-zinc-200">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            <button type="button" className="px-3 py-2 rounded bg-indigo-100 text-indigo-700 text-sm" onClick={() => setShowNotes(true)}>📓 Notes</button>
            <button type="button" onClick={onClose} className="text-zinc-500 hover:text-zinc-700">✕</button>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-zinc-200 p-4 bg-white/80">
          {total === 0 ? (
            <div className="text-sm text-zinc-600">No cards yet. Add a few below to start.</div>
          ) : (
            <>
              <div className="flex items-center justify-between mb-2 text-sm text-zinc-600">
                <div>Card {idx + 1} / {total}</div><div>Correct: {correct}</div>
              </div>
              ...
              <div className="mt-3 flex gap-2">
                {!showAns ? (
                  ...
                  <>
                    <button type="button" onClick={() => mark(false)} className="px-4 py-2 rounded border border-zinc-300 text-sm">Again</button>
                    <button type="button" onClick={() => mark(true)} className="px-4 py-2 rounded bg-emerald-600 text-white text-sm">Correct</button>
                  </>
                )}
              </div>
            </>
        ...
        <div className="mt-5">
          ...
          <div className="mt-2 flex justify-end">
            <button type="button" onClick={addCard} className="px-4 py-2 rounded bg-zinc-900 text-white text-sm">Add Card</button>
          </div>
        </div>
...
export default function VSCatPetApp() {
  ...
  // Pet size/placement
  const PET_SCALE = 5.0;
  const PET_BASE = { w: 56, h: 40 };
  const PET_W = PET_BASE.w * PET_SCALE;
  const PET_H = PET_BASE.h * PET_SCALE;
  ...
  // Global Notes panel flag
  const [showNotes, setShowNotes] = useState(false);

  // Persistent pets (with migration)
  const [pets, setPets] = useLocalStorage("pets.list", (() => {
    try {
      const old = JSON.parse(localStorage.getItem("pet.meta") || "null");
      const extra = JSON.parse(localStorage.getItem("pet.extra") || "{}");
      if (old) {
        return [{
          id: String(Date.now()),
          species: old.species || "cat",
          avatar: "🐱",
          name: old.name || "Rupert",
          createdAt: old.createdAt ?? Date.now(),
          hearts: old.hearts ?? 0,
          energy: old.energy ?? 0,
          memDate: old.memDate || todayStr(),
          memCorrect: old.memCorrect ?? 0,
          memTotal: old.memTotal ?? 0,
          weight: old.weight ?? 1.0,
          distanceKm: old.distanceKm ?? 0,
          goals: old.goals ?? [],
          extra: extra || {}
        }];
      }
  ...
  const setPet = (next) => {
    setPets(list => {
      const copy = [...list];
      copy[activeIndex] = typeof next === "function" ? next(copy[activeIndex]) : next;
      return copy;
    });
  };
  ...
  useEffect(() => {
    ...
    const loop = (t) => {
      ...
      setPos(p => {
        ...
        let tgt = target; let speed = SPEED_IDLE; roamTimer.current -= dt;
        if (!tgt) {
          if (roamTimer.current <= 0) {
            tgt = { x: rand(b.left + EDGE_PADDING, b.right - EDGE_PADDING), y: rand(b.top + EDGE_PADDING, b.bottom - EDGE_PADDING) };
            roamTimer.current = rand(2, 4);
          }
        } else {
          speed = SPEED_CHASE;
        }
        const local = { x: p.x, y: p.y };
        const lt = tgt ? { x: tgt.x - b.left, y: tgt.y - b.top } : local;
        const next = stepToward(local, lt, speed, dt);
        ...
        if (tgt && Math.hypot(lt.x - nx, lt.y - ny) < 8) {
          setTarget(null); setBall(bal => bal ? { ...bal, ttl: 0.3 } : bal);
          setShowHeart(true); setTimeout(() => setShowHeart(false), 400);
        }
        return { x: nx, y: ny };
      });
      setBall(bal => bal ? { ...bal, ttl: Math.max(0, (bal.ttl ?? 1) - dt) } : bal);
      raf = requestAnimationFrame(loop);
    };
    raf = requestAnimationFrame(loop);
    return () => cancelAnimationFrame(raf);
  }, [target, facingLeft, setPet]);
  ...
  // CRUD helpers
  const createPet = (name, species, extras) => {
    const now = Date.now(); const id = String(now);
    const newPet = { id, name: name || "Unnamed", species, avatar: speciesAvatar(species), createdAt: now, hearts: 0, energy: 0, memDate: todayStr(), memCorrect: 0, memTotal: 0, weight: 1.0, distanceKm: 0, goals: [], extra: extras || {} };
    setPets(list => [...list, newPet]); setActiveId(id); setShowNewPet(false); setShowInfo(true); placePetCenterBottom();
  };
  ...
  const deletePet = (id) => setPets(list => {
    const next = list.filter(p => p.id !== id); if (next.length === 0) return list;
    if (activeId === id) setActiveId(next[0].id); return next;
  });
  const movePet = (id, dir) => setPets(list => {
    const i = list.findIndex(p => p.id === id); if (i < 0) return list;
    const ni = clamp(i + (dir === "up" ? -1 : 1), 0, list.length - 1); if (ni === i) return list;
    const copy = [...list]; const [it] = copy.splice(i, 1); copy.splice(ni, 0, it); return copy;
  });
  ...
  return (
    <div className="min-h-screen w-full text-zinc-100 bg-zinc-900">
      ...
      <div
        ref={wrapRef}
        onClick={onClick}
        className="relative z-0 w-full h-[200px] md:h-[690px] overflow-hidden bg-cover bg-no-repeat"
      ...
      >
        ...
        <button
          ...
          style={{ left: `${MEM_HOTSPOT.cx}%`, top: `${MEM_HOTSPOT.cy}%`, width: `${MEM_HOTSPOT.w}%`, height: `${MEM_HOTSPOT.h}%`, minWidth: "180px", minHeight: "120px", transform: "translate(-50%, -50%)", cursor: "pointer", zIndex: 5 }}
        />
        <button
          type="button"
          ...
          style={{ left: `${NOTES_HOTSPOT.cx}%`, top: `${NOTES_HOTSPOT.cy}%`, width: `${NOTES_HOTSPOT.w}%`, height: `${NOTES_HOTSPOT.h}%`, minWidth: "160px", minHeight: "110px", transform: "translate(-50%, -50%)", cursor: "pointer", zIndex: 5 }}
        />
        <button
          type="button"
          ...
          style={{ left: `${GOAL_HOTSPOT.cx}%`, top: `${GOAL_HOTSPOT.cy}%`, width: `${GOAL_HOTSPOT.w}%`, height: `${GOAL_HOTSPOT.h}%`, minWidth: "180px", minHeight: "120px", transform: "translate(-50%, -50%)", cursor: "pointer", zIndex: 5 }}
        />
        <button
          type="button"
          ...
          style={{ left: `${FLASH_HOTSPOT.cx}%`, top: `${FLASH_HOTSPOT.cy}%`, width: `${FLASH_HOTSPOT.w}%`, height: `${FLASH_HOTSPOT.h}%`, minWidth: "160px", minHeight: "110px", transform: "translate(-50%, -50%)", cursor: "pointer", zIndex: 5 }}
        />
        ...
        <div className="absolute" style={{ left: pos.x - PET_W / 2, top: pos.y - PET_H / 2, width: PET_W, height: PET_H }} role="img" aria-label={`${pet.species} pet`}>
          ...
          {showHeart && <div className="absolute -top-3 left-1/2 -translate-x-1/2 text-pink-400 text-xl select-none">❤</div>}
        </div>

        {/* hint */}
        <div className="absolute bottom-3 right-3 text-xs text-amber-200 bg-black/40 px-3 py-1.5 rounded-full backdrop-blur">
          Click to throw a ball · Hover to pet · ℹ️ for details
        </div>
      </div>
